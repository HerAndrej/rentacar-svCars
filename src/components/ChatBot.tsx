...
export default function ChatBot() {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50 w-[calc(100vw-2rem)] sm:w-[400px] h-[calc(100vh-6rem)] sm:h-[560px] bg-bg-primary border border-border rounded-2xl shadow-2xl shadow-black/50 flex flex-col overflow-hidden">
          {/* Header */}
          <div className="bg-bg-secondary border-b border-border px-4 sm:px-5 py-3 sm:py-4 flex items-center justify-between flex-shrink-0">
              ...
              <div className="flex flex-wrap gap-2 pt-1">
                {quickActions.map((action) => (
                  <button
                    ...
                    className="px-3 py-1.5 bg-bg-card border border-border rounded-full text-xs text-text-secondary hover:border-accent/50 hover:text-accent transition-all"
                  >
                    {action}
                  </button>
                ))}
          ...
          {/* Input */}
          <form onSubmit={sendMessage} className="border-t border-border p-3 flex gap-2 flex-shrink-0">
            <input
              ref={inputRef}
              type="text"
              value={input}
              ...
              disabled={loading}
              className="flex-1 bg-bg-card border border-border rounded-xl px-4 py-2.5 text-sm text-text-primary placeholder:text-text-muted focus:outline-none focus:border-accent/50 disabled:opacity-50"
            />
            <button
              type="submit"
